 ...
 * the same ratio as the bounds for the tree. The bound here is the boundary
 * <p/>
...
public class QuadTree {
	...
	/*
	 * knn search based on quad-tree.
	 */
	public void knn(Point point, int topk){
		...
		while (queue.size() != 0){
			...
			e = (Node)first.m_pEntry;
			if(e.getNodeType()==NodeType.LEAF){
				if(list.size()<topk)
					list.add(first.m_minDist);
				else{
					if(first.m_minDist>list.indexOf(topk-1))
						break;
					else{
						list.remove(topk-1);
						list.add(first.m_minDist);
			...
			}else if(e.getNodeType()==NodeType.POINTER){
				for (int cChild = 0; cChild < 4; cChild++)
				{
					...
					if(cChild == 0)
						...
						child = e.getSw();					
					double mind = 0;
					if(child.getNodeType() == NodeType.POINTER)
						mind = distanceNodePoint(child, point);
					else
						mind = distancePointPoint(child, point);
	...
	public double distanceNodePoint(Node child, Point point){
		double x = child.getX();
		double y = child.getY();
		double width = child.getW();
		double mind = 0;
		if(Math.abs(x-point.getX())<width && Math.abs(y-point.getY())<width)
			mind = 0.0;
		else if(Math.abs(x-point.getX())<width && Math.abs(y-point.getY())>width)
			mind = Math.abs(Math.abs(y-point.getY())-width);
		else if(Math.abs(x-point.getX())>width && Math.abs(y-point.getY())<width)
			mind = Math.abs(Math.abs(x-point.getX())-width);
		else
			mind = Math.sqrt(Math.pow(Math.abs(x-point.getX())-width, 2)
				+Math.pow(Math.abs(y-point.getY())-width, 2));
		return mind;
	}
	public double distancePointPoint(Node child, Point point){
		double x = child.getX();
		double y = child.getY();
		double mind = Math.sqrt(Math.pow(Math.abs(x-point.getX()), 2)
				+Math.pow(Math.abs(y-point.getY()), 2));
		return mind;
	}
	 ...
	 * pruning for a query
	 */
	public double RkNN(Point query[], int topk){
		PriorityQueue queue = new PriorityQueue(100, new NNEntryComparator());		
		Node e = root_;
		queue.add(new NNEntry(e, 0.0));
		ArrayList<Node> sFilter = new ArrayList<>();
		ArrayList<Point> sRefine = new ArrayList<>();
		
		ArrayList<Double> list = new ArrayList<>();
		while (queue.size() != 0){
			NNEntry first = (NNEntry) queue.poll();
			e = (Node)first.m_pEntry;
			if(trimNode(query, sRefine, e)){
				sFilter.add(e);
			}
			else {
				if (e.getNodeType() == NodeType.LEAF) {
					Point aPoint = e.getPoint(); // the only point here.
					if(trimPoint(query, sRefine, aPoint)){
						sRefine.add(aPoint);
					}
					...
					for (int cChild = 0; cChild < 4; cChild++) {
						...
						double mind = 0;
					/*	if (child.getNodeType() == NodeType.POINTER)
							mind = distanceNodePoint(child, point[0]);
						else
							mind = distancePointPoint(child, point[0]);*/
						queue.add(new NNEntry(child, mind));
					}
				} else{
	...
	/*
	 * build the counter of each node based on trajectory.
	 * We 
	 */
	public void buildInvertedCounter(){
		
	}
	
	/*
	 * check whether the node is covered by the filtered area.
	 */
	public boolean trimNode(Point query[], ArrayList<Point> filter, Node node){
		Node nodeResM = node;
		for(Point filterpoint: filter){
			//form the bisectors with query
			for(int i = 0; i<query.length; i++)
			{
				Point querypoint = query[i];
				Rectangle r1 = new Rectangle(100, 100, 100, 100);
				Line2D l1 = new Line2D.Float(0, 200, 200, 0);//bisector between query point and filterpoint
				if(l1.intersects(r1)){
					;
	...
	public boolean trimPoint(Point query[], ArrayList<Point> filter, Point point){
		for(Point filterpoint: filter){
			//form the bisectors with query
			for(int i = 0; i<query.length; i++)
			{
