package au.edu.rmit.csit.RkNNT.mtree;
...
import au.edu.rmit.csit.RkNNT.mtree.SplitFunction.SplitResult;
...
public class MTree<DATA> {
						...
						if (Math.abs(pending.distance - child.distanceToParent) - child.radius <= Query.this.range) {
							...
							if (childMinDistance <= Query.this.range) {
								if (child instanceof MTree.Entry) {
									...
									nearestQueue
											.add(new ItemWithDistances<Entry>(entry, childDistance, childMinDistance));
								} else {
									@SuppressWarnings("unchecked")
									Node childNode = (Node) child;
									pendingQueue.add(
											new ItemWithDistances<Node>(childNode, childDistance, childMinDistance));
								}
							}
						}
					}
	...
	/*
	 * this algorithm is based on the TKDE paper
	 * "Reverse Nearest Neighbor Search in Metric Spaces"
	 */
	public class RkNNT implements Iterable<ResultItem> {
		private RkNNT(DATA data, int limit) {
			this.data = data;
			this.limit = limit;
		}

		public void filter(IndexItem Node1, Node parent) {
			System.out.println(Node1.radius);
			Map<IndexItem, Double> childDistance = new HashMap<>();// sort the child in a node
			Node node = (Node) Node1;
			if (parent != null) {// not root
				for (IndexItem child : node.children.values()) {
					childDistance.put(child, child.distanceToParent);
				}
				childDistance = sortByValue(childDistance);// ascending order
				if (MTree.this.distanceFunction.calculate(RkNNT.this.data, parent.data) >= (2 * parent.radius
						+ childDistance.get(childDistance.keySet().toArray()[limit - 1])))
					return;
			}
			if (Node1 instanceof MTree.Entry) {// leaf node
				Map<IndexItem, Boolean> prune_lable = new HashMap<IndexItem, Boolean>();
				int count = 0;
				System.out.print("ssssssss");
				for (IndexItem child : childDistance.keySet()) {
					double max = 0;// lemma 3
					if (limit == 1) {
						if (count == 0)
							max = childDistance.get(childDistance.keySet().toArray()[1]);
						else
							max = childDistance.get(childDistance.keySet().toArray()[count]);
					} else {
						if (count >= limit) {
							max = childDistance.get(childDistance.keySet().toArray()[count])
									+ childDistance.get(childDistance.keySet().toArray()[limit - 1]);
							...
							max = childDistance.get(childDistance.keySet().toArray()[count])
									+ +childDistance.get(childDistance.keySet().toArray()[limit]);
						}
					}
					count++;
					double Distance = MTree.this.distanceFunction.calculate(RkNNT.this.data, parent.data);
					if (Math.abs(Distance - child.distanceToParent) >= max) {// Rule
																				// 6
						prune_lable.put(child, false);
						continue;
					}
					if (MTree.this.distanceFunction.calculate(child.data, RkNNT.this.data) >= max)// Rule
																									// 5
						prune_lable.put(child, false);
					prune_lable.put(child, true);
				}
				for (IndexItem child : prune_lable.keySet()) {
					if (prune_lable.get(child)) {
						// whether they are more close to k other objects.
						s_can.add(new ResultItem(child.data, child.radius));
					}
				}
			} else {// not leaf node
				for (IndexItem child : node.children.values()) {
					double lamda = 0;
					if (limit == 1) {
						lamda = 2 * child.radius;
					} else {
						lamda = 3 * child.radius;
					}
					double Distance = MTree.this.distanceFunction.calculate(RkNNT.this.data, node.data);
					if (Math.abs(Distance - child.distanceToParent) >= lamda) {
						continue;
					}
					if (Distance >= lamda)
						continue;
					filter(child, node);// iteration to filer the child
				}
			}
		}
		 ...
		 */
		public void refine() {
			System.out.println(s_can.size());
			Iterator<ResultItem> sIterator = s_can.iterator();
			while (sIterator.hasNext()) {
				ResultItem ri1 = sIterator.next();
				ArrayList<Node> S = new ArrayList<>();// result
				Map<Node, Double> nodeDistance = new HashMap<>();// min-heap
				nodeDistance.put(MTree.this.root, Double.POSITIVE_INFINITY);
				while (!nodeDistance.isEmpty()) {
					Iterator<Node> iterator = nodeDistance.keySet().iterator();
					while (iterator.hasNext()) {
						Node eNode = iterator.next();
						nodeDistance.remove(eNode);
						for (IndexItem child : eNode.children.values()) {
							if (eNode.children != null) {
								double childDistance = MTree.this.distanceFunction.calculate(RkNNT.this.data,
										eNode.data);
								if (childDistance <= ri1.distance)
									S.add(eNode);
								if (childDistance + eNode.radius <= ri1.distance)
									s_can.remove(ri1);
								if (childDistance - eNode.radius <= ri1.distance) {
									double minD = 0;
									nodeDistance.put(eNode, minD);//
								}
								nodeDistance = sortByValue(nodeDistance);
							} else {
								double childDistance = MTree.this.distanceFunction.calculate(RkNNT.this.data,
										eNode.data);
								if (childDistance <= ri1.distance)
		...
		private DATA data;
		private int limit;
		private ArrayList<ResultItem> s_can = new ArrayList<>();
	...
	public MTree(int minNodeCapacity, int maxNodeCapacity, DistanceFunction<? super DATA> distanceFunction,
		...
		if (splitFunction == null) {
			splitFunction = new ComposedSplitFunction<DATA>(new PromotionFunctions.RandomPromotion<DATA>(),
	...
	/**
	 * knn search Performs a nearest-neighbors query on the M-Tree, constrained
	...
	/**
	 * Performs a reverse k nearest-neighbor query on the M-Tree, constrained by
	 ...
	 * @return A {@link Query} object used to iterate on the results.
	 */
	public RkNNT getRkNNT(DATA queryData, int limit) {
		return new RkNNT(queryData, limit);
	}
